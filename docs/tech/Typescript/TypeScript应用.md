...
```tsx
{
  ...
  console.log(Direct.Up); //0  打印索引
  console.log(Direct[0]); //'Up'  打印值
}
```
# 16. 类
## 1. 属性和方法
...
```tsx
  ...
  let a = new Animal("Jack");
  console.log(a.name); // 报错 不能在类Animal的外部访问
  a.say();
  //   a.name = "Tom";
  ...
  class Cat extends Animal {
    ...
    run() {
      // 报错： 因为name是父类中用private定义的私有属性，所以不能在类Animal的外部访问
      console.log(this.name + "在跑");
    }
  }
  let cat = new Cat("mao", "gray");
...
```tsx
  ...
  class Animal {
    protected name;  // 可以在子类中调用 ，但是不能在外部用实例直接调用 
    public constructor(name) {
      this.name = name;
    }
  }
